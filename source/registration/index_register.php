...
$sql_sections = "SELECT ID, Nazva FROM Section";
...
<body class="register">
    <div class="container">
        ...
        <form class="reg" id="registration-form" action="/source/registration/index_register_process.php" method="POST">
            ...
            <label for="sections">Виберіть секції:</label>
            <div id="sections">
                <?php
                while ($row = $result_sections->fetch_assoc()) {
                    echo "<button type='button' class='section-button' data-id='" . $row['ID'] . "'>" . $row['Nazva'] . "</button>";
                }
                ?>
            </div>

            <input type="hidden" name="section_id" id="selected-sections" value="">
    ...
    <script>
        ...
        const selectedSectionsInput = document.getElementById('selected-sections');
